...
отвод камеры, дистанция тумана, русский язык.
...
- **Определяет, что уже установлено.** При запуске читает реальное состояние игры
  (пропатчены ли DLL, стоит ли UI-мод и зум, какие локали есть, что в
...
- **«Запустить игру»** — доступна всегда, стартует игру как есть, ничего не применяя.
...
(картинка шапки, ширина 688 × высота 84). Оба файла лежат в репозитории, иконка
вшивается в `.exe` при сборке. Без них приложение работает как обычно — в шапке
просто будет текст.
...
.\DoW-Launcher.ps1 -LocaleInfo   # какие локали реально стоят в игре
...
- **жёсткая обрезка** — стандартные текстуры игры: они обрезаны по границе с
  картой, части раздвигаются по углам, и на стыках виден резкий обрыв картинки
  когда-то цельной панели. **Сейчас это единственный режим.**
- **дорисованные** — *в разработке, выбор в менеджере заблокирован.* Идея:
  панель с плавным, аккуратным краем вместо резкого среза.

Готового перерисованного арта здесь нет и не будет: он делается на основе
игровых текстур, а это чужой контент — раздавать его вместе с модом нельзя.

Механика при этом на месте, так что свой арт можно подключить вручную:

```powershell
.\ui-unstretch\Edit-BarTextures.ps1 -Export    # выгрузить ломтики в PNG
.\ui-unstretch\Edit-BarTextures.ps1 -Preview   # проверить совмещение без игры
.\ui-unstretch\Edit-BarTextures.ps1 -Import    # вернуть правки в игру
```

Полностью перерисованные ломтики любого размера кладите в
`ui-unstretch\textures-custom\<раса>\<имя ломтика>.png` (папка в `.gitignore`).
При `-Import` фон, связанный с краями картинки, вырезается заливкой, а арт
подгоняется по габаритам оригинального ломтика — кнопки и мини-карта остаются
на своих местах.

**Внутренние дыры панели** (командная карта, гнёзда портретов) должны быть
прозрачными, чтобы сквозь них был виден мир и кнопки. Заливка от краёв до них
не доходит — они заперты рамкой. Сделать их прозрачными:

- нарисовать в редакторе настоящую прозрачность и выключить кеинг:
  `align.json` → `{"threshold":-1}`; либо
...
> Модуль пока не подключён к окну менеджера — запускайте из консоли.
